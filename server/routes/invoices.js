...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const invoiceLink = `/dashboard?tab=bookings&booking=${bookingId}&invoice=${newInvoice._id}`;
    await createNotification(
      booking.buyerId,
      'invoice',
      `You have received an invoice for booking #${bookingId.substring(0, 6)}`,
      invoiceLink,
      {
...
router.put('/:id/status', auth, async (req, res) => {
  try {
    ...
    const invoiceLink = `/dashboard?tab=bookings&booking=${invoice.bookingId}&invoice=${invoice._id}`;
    await createNotification(
      recipientUserId,
      'invoice_status',
      `Invoice #${invoice.invoiceNumber} has been marked as ${status}`,
      invoiceLink,
      {
