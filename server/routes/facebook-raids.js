...
router.post('/points', auth, requireEmailVerification, async (req, res) => {
  try {
    ...
    // Multi-group users can pick which group is default via /mygroups.
...
// Create a new free Facebook raid (for lifetime bumped projects or manually approved free raid projects)
  ...
  try {
    ...
    if (!completion.pointsAwarded) {
      ...
      if (user) {
        ...
        const isVerifiedBonus = req.body.points === 15 || req.body.points === 20;
        user.points += points;
