...
// Start ID verification (user submits verification)
router.post('/start-id-verification', auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId);
    ...
    user.idVerification = {
      status: 'pending',
      stripeVerificationUrl: 'https://verify.stripe.com/v/eVq00jgmI3mA1DJdIld3i00',
    ...
    const { emitIdVerificationStatusUpdate } = require('../socket');
    emitIdVerificationStatusUpdate(user._id, user.idVerification);

    res.json({ 
      message: 'ID verification started successfully',
      verificationUrl: user.idVerification.stripeVerificationUrl
    });
  } catch (error) {
    console.error('Start ID verification error:', error);
    res.status(500).json({ error: 'Failed to start ID verification' });
  }
});

// Note: Pending ID verifications are now handled via socket events
// This endpoint is kept for backward compatibility but not used in the frontend

// Approve ID verification (admin only)
router.post('/:userId/approve-id-verification', auth, async (req, res) => {
  try {
    if (!req.user.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    }
    ...
    if (user.idVerification.status !== 'pending') {
      return res.status(400).json({ error: 'User ID verification is not pending' });
    }

    // Update verification status
    user.idVerification.status = 'verified';
    user.idVerification.reviewedAt = new Date();
    user.idVerification.reviewedBy = req.user.userId;
    ...
    await Notification.createNotification({
      userId: user._id,
      type: 'status',
      message: 'Your ID verification has been approved! You now have the "ID Verified" badge.',
      relatedModel: 'User'
    });

    // Emit socket events for real-time updates
    const { emitIdVerificationApproved, emitIdVerificationStatusUpdate } = require('../socket');
    emitIdVerificationApproved(user._id, user.idVerification);
    emitIdVerificationStatusUpdate(user._id, user.idVerification);
...
router.post('/:userId/reject-id-verification', auth, async (req, res) => {
  try {
    if (!req.user.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    }
    ...
    user.idVerification.reviewedAt = new Date();
    user.idVerification.reviewedBy = req.user.userId;
    user.idVerification.rejectionReason = rejectionReason || 'Verification failed to meet requirements';
    ...
    await Notification.createNotification({
      userId: user._id,
      type: 'status',
      message: `Your ID verification was rejected. Reason: ${user.idVerification.rejectionReason}. You can try again.`,
      relatedModel: 'User'
    });

    // Emit socket events for real-time updates
    const { emitIdVerificationRejected, emitIdVerificationStatusUpdate } = require('../socket');
    emitIdVerificationRejected(user._id, user.idVerification);
    emitIdVerificationStatusUpdate(user._id, user.idVerification);
    ...
    res.status(500).json({ error: 'Failed to reject ID verification' });
  }
});
