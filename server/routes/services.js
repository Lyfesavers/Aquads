...
// NEW: Optimized route to get services with reviews in single query (eliminates N+1 problem)
router.get('/optimized/with-reviews', async (req, res) => {
  try {
    const { category, sort, limit = 20, page = 1 } = req.query;
    ...
    // Use aggregation pipeline to get services with review counts and average ratings
    const pipeline = [
      { $match: query },
      {
        $lookup: {
      ...
      {
        $project: {
          reviewDetails: 0 // Remove the full review details to keep response small
        }
      },
      { $sort: sortOptions },
      { $skip: (parseInt(page) - 1) * parseInt(limit) },
