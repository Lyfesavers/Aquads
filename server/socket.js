...
const { attachBeanstalksChutes } = require('./beanstalksChutes');
const { attachLudo } = require('./ludo');
...
function init(server) {
  ...
  io.on('connection', (socket) => {
    attachBeanstalksChutes(socket, io);
    attachLudo(socket, io);
      ...
      if (userData && userData.userId) {
        try {
          ...
          const earningsSummary = {
            ...
            nextTier:
              !user.isVipAffiliate && currentRate === 0.10
                ? {
                    rate: 0.15,
                    amountNeeded: 5000,
...
function emitRaidUpdate(type, raid, platform) {
  if (!io) {
    return;
  }
  
  // type: 'created' or 'cancelled'
