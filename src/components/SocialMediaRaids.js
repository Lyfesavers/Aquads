                ...
                <form onSubmit={safeHandleSubmit} id="verification-form">
                  ...
                  {iframeVerified && (
                    <div className="mb-6 p-4 bg-green-500/20 border border-green-500 rounded-lg">
                      ...
                      <p className="text-gray-300 mt-2">You can now complete the task to earn your points.</p>
                    </div>
                  )}
                  
                  <button
                    type="submit"
                    className={`w-full mt-6 py-3 px-4 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors ${
                        ...
                        if (!previewState.tweetId) {
                          ...
                          if (tweetId) {
                            ...
                            setPreviewState(prev => ({
                              ...
                              loading: false,
                              error: false,
                              message: 'Tweet ready for interaction',
                              tweetId
                            }));
                          } else {
              ...
              {previewState.tweetId ? (
                <>
                  ...
                  <div className="mt-4 bg-gray-800/70 rounded p-3 border border-gray-700">
                    <h5 className="text-gray-300 font-medium mb-2">How to Complete:</h5>
                                  ...
                                  {iframeLoading ? (
                                    ...
                                    <>
                                      <div className="bg-gray-800 rounded-lg p-4 mb-4 text-center">
                                        <h3 className="font-bold text-lg text-white mb-2">Tweet Interaction Verification</h3>
                                        <p className="text-gray-400 mb-3 text-sm">Click each button to interact with the tweet. After clicking, you will be redirected to Twitter to complete the action.</p>
                                        <div className="flex justify-center text-blue-400">
                                          <a 
                                            href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(selectedRaid?.tweetUrl || tweetUrl)}`} 
                                            target="_blank" 
                                      ...
                                      <div className="grid grid-cols-3 gap-3">
                                        {/* Like button */}
                                        <div className={`${iframeInteractions.liked ? 'bg-green-500/20 border-green-500' : 'bg-gray-800 border-gray-700'} border rounded-lg p-3 text-center transition-colors`}>
                                          <button 
                                            onClick={() => {
                                              window.open(`https://twitter.com/intent/like?tweet_id=${previewState.tweetId}`, '_blank');
                                              handleIframeInteraction('liked');
                                            ...
                                            className={`${iframeInteractions.liked ? 'opacity-70 cursor-default' : 'hover:text-pink-500'} w-full flex flex-col items-center`}
                                          >
                                            <svg className={`w-6 h-6 mb-1 ${iframeInteractions.liked ? 'text-pink-500' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 24 24">
                                              <path d="M12 21.638h-.014C9.403 21.59 1.95 14.856 1.95 8.478c0-3.064 2.525-5.754 5.403-5.754 2.29 0 3.83 1.58 4.646 2.73.814-1.148 2.354-2.73 4.645-2.73 2.88 0 5.404 2.69 5.404 5.755 0 6.376-7.454 13.11-10.037 13.157H12z"/>
                                            </svg>
                                            <span className={`${iframeInteractions.liked ? 'text-green-400' : 'text-gray-300'} font-medium text-xs sm:text-sm`}>
                                              {iframeInteractions.liked ? 'Liked ✓' : 'Like'}
                                            </span>
                                          </button>
                                        </div>
                                        ...
                                        {/* Retweet button */}
                                        <div className={`${iframeInteractions.retweeted ? 'bg-green-500/20 border-green-500' : 'bg-gray-800 border-gray-700'} border rounded-lg p-3 text-center transition-colors`}>
                                          <button 
                                            onClick={() => {
                                              window.open(`https://twitter.com/intent/retweet?tweet_id=${previewState.tweetId}`, '_blank');
                                              handleIframeInteraction('retweeted');
                                            ...
                                            className={`${iframeInteractions.retweeted ? 'opacity-70 cursor-default' : 'hover:text-green-500'} w-full flex flex-col items-center`}
                                          >
                                            <svg className={`w-6 h-6 mb-1 ${iframeInteractions.retweeted ? 'text-green-500' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 24 24">
                                              <path d="M23.77 15.67c-.292-.293-.767-.293-1.06 0l-2.22 2.22V7.65c0-2.068-1.683-3.75-3.75-3.75h-5.85c-.414 0-.75.336-.75.75s.336.75.75.75h5.85c1.24 0 2.25 1.01 2.25 2.25v10.24l-2.22-2.22c-.293-.293-.768-.293-1.06 0s-.294.768 0 1.06l3.5 3.5c.145.147.337.22.53.22s.383-.072.53-.22l3.5-3.5c.294-.292.294-.767 0-1.06zm-10.66 3.28H7.26c-1.24 0-2.25-1.01-2.25-2.25V6.46l2.22 2.22c.148.147.34.22.532.22s.384-.073.53-.22c.293-.293.293-.768 0-1.06l-3.5-3.5c-.293-.294-.768-.294-1.06 0l-3.5 3.5c-.294.292-.294.767 0 1.06s.767.293 1.06 0l2.22-2.22V16.7c0 2.068 1.683 3.75 3.75 3.75h5.85c.414 0 .75-.336.75-.75s-.337-.75-.75-.75z"></path>
                                            </svg>
                                            <span className={`${iframeInteractions.retweeted ? 'text-green-400' : 'text-gray-300'} font-medium text-xs sm:text-sm`}>
                                              {iframeInteractions.retweeted ? 'Retweeted ✓' : 'Retweet'}
                                            </span>
                                          </button>
                                        </div>
                                        ...
                                        {/* Reply button */}
                                        <div className={`${iframeInteractions.commented ? 'bg-green-500/20 border-green-500' : 'bg-gray-800 border-gray-700'} border rounded-lg p-3 text-center transition-colors`}>
                                          <button 
                                            onClick={() => {
                                              window.open(`https://twitter.com/intent/tweet?in_reply_to=${previewState.tweetId}`, '_blank');
                                              handleIframeInteraction('commented');
                                            ...
                                            className={`${iframeInteractions.commented ? 'opacity-70 cursor-default' : 'hover:text-blue-500'} w-full flex flex-col items-center`}
                                          >
                                            <svg className={`w-6 h-6 mb-1 ${iframeInteractions.commented ? 'text-blue-500' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 24 24">
                                              <path d="M14.046 2.242l-4.148-.01h-.002c-4.374 0-7.8 3.427-7.8 7.802 0 4.098 3.186 7.206 7.465 7.37v3.828c0 .108.044.286.12.403.142.225.384.347.632.347.138 0 .277-.038.402-.118.264-.168 6.473-4.14 8.088-5.506 1.902-1.61 3.04-3.97 3.043-6.312v-.017c-.006-4.367-3.43-7.787-7.8-7.788zm3.787 12.972c-1.134.96-4.862 3.405-6.772 4.643V16.67c0-.414-.335-.75-.75-.75h-.396c-3.66 0-6.318-2.476-6.318-5.886 0-3.534 2.768-6.302 6.3-6.302l4.147.01h.002c3.532 0 6.3 2.766 6.302 6.296-.003 1.91-.942 3.844-2.514 5.176z"></path>
                                            </svg>
                                            <span className={`${iframeInteractions.commented ? 'text-green-400' : 'text-gray-300'} font-medium text-xs sm:text-sm`}>
                                              {iframeInteractions.commented ? 'Replied ✓' : 'Reply'}
                                            </span>
                                          </button>
                                        </div>
