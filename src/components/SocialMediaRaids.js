...
const SocialMediaRaids = ({ currentUser, showNotification }) => {
  ...
  const handleRaidClick = (raid) => {
    ...
    // Don't reset Twitter username - keep the saved one for convenience
    // setTwitterUsername('');
  ...
  const checkFreeRaidEligibility = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/affiliates/free-raid-project/${currentId}`, {
        headers: {
          'Authorization': `Bearer ${currentUser.token}`
        }
      });
  ...
  const handleCreateFreeRaid = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/twitter-raids/free`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${currentUser.token}`
