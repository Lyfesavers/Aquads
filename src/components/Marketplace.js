...
const Marketplace = ({ currentUser, onLogin, onLogout, onCreateAccount, onBannerSubmit, openMintFunnelPlatform }) => {
  ...
  const handleCreateAccountSubmit = async (formData) => {
    try {
      ...
      // Redirect to home page so user can see the verification modal/notifications
      // The verification modal will be shown by App.js on the home page
      navigate('/home');
    } catch (error) {
    ...
    <div className="h-screen overflow-y-auto bg-gradient-to-br from-gray-900 to-black text-white">
      ...
      <div className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          {/* How It Works - Only show for Freelancer Services, not for Jobs */}
          {!showJobs && (
                      ...
                      >
                        <div className="aspect-w-16 aspect-h-9 relative">
                          ...
                          {currentUser && service.seller?.username === currentUser.username && (
                            <div className={`absolute right-2 flex gap-2 z-[200000] ${service.isPremium ? 'top-12' : 'top-2'}`}>
                              {!service.isPremium && (
                                <button
