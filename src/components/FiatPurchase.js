...
// Transak configuration with staging API
const TRANSAK_CONFIG = {
  apiKey: process.env.REACT_APP_TRANSAK_API_KEY || 'staging_api_key_here', // Replace with your Transak staging key
  environment: process.env.REACT_APP_TRANSAK_ENVIRONMENT || 'STAGING', // STAGING or PRODUCTION
  themeColor: '00d4ff',
...
const FiatPurchase = ({ userWallet, showNotification }) => {
  ...
  const initializeTransak = () => {
    ...
    const transakConfig = {
      ...
      cryptoCurrencyList: POPULAR_CRYPTOS.map(crypto => crypto.symbol).join(','),
      fiatCurrencyList: FIAT_CURRENCIES.map(fiat => fiat.code).join(','),
      email: '', // Users will enter this in Transak
      userData: {
        firstName: '',
        lastName: '',
        email: '',
        ...
        <div className="header-content">
          ...
          {TRANSAK_CONFIG.environment === 'STAGING' && (
            <div className="staging-notice">
              <span className="staging-badge">STAGING MODE</span>
              <p>Testing environment - No real transactions</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="purchase-form">
        ...
        <div className="fee-info">
          ...
          <p className="fee-note">
            ...
            {TRANSAK_CONFIG.environment === 'STAGING' && ' (Test mode - no real charges)'}
          </p>
        </div>
