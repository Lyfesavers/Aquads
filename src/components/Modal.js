...
const Modal = ({ children, onClose, fullScreen = false }) => {
  ...
  if (fullScreen) {
    return (
      <>
        ...
        <div 
          className="fixed inset-0 bg-gradient-to-br from-gray-900 to-black z-[99999999] modal-backdrop"
  ...
  return (
    <>
      ...
      <style>{modalStyles}</style>
      <div 
        className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[99999999] p-2 sm:p-4 overflow-y-auto modal-backdrop"
        onClick={handleBackdropClick}
      >
        <div 
