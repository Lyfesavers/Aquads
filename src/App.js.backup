...
function App() {
  ...
  useEffect(() => {
    const loadAdsFromApi = async () => {
      try {
        ...
        // Fetch vote counts for all ads
        await fetchVoteCounts(repositionedAds);
      } catch (error) {
  ...
  // Fetch vote counts for all ads
  const fetchVoteCounts = async (adsToCheck) => {
    const votePromises = adsToCheck.map(async (ad) => {
      try {
        const voteCount = await getAdVoteCount(ad.id);
        return { id: ad.id, voteCount };
      } catch (error) {
        console.error(`Error fetching votes for ad ${ad.id}:`, error);
        return { id: ad.id, voteCount: 0 };
      }
    });

    const voteCounts = await Promise.all(votePromises);
    const votesObject = voteCounts.reduce((acc, { id, voteCount }) => {
      acc[id] = voteCount;
      return acc;
    }, {});
  ...
  // Modify the createBubble function to check for votes and add vote counter
  const createBubble = (ad) => {
    ...
    voteCounter.id = `vote-counter-${ad.id}`;
    ad.element.appendChild(voteCounter);
    
    return ad;
  };

  // Add a function to update vote counter
  const updateVoteCounter = (adId, voteCount) => {
    const counter = document.getElementById(`vote-counter-${adId}`);
    if (counter) {
      counter.textContent = `${voteCount} votes`;
  ...
  // Update handler for bubble double-click
  const handleBubbleDoubleClick = async (adId) => {
    try {
      ...
      if (result.success) {
        ...
        // Show notification
        showNotification('Vote recorded! Bubble size increased.', 'success');
        ...
        if (adElement) {
          ...
          updateVoteCounter(adId, result.voteCount);
          
          // Add pulse animation to bubble
          adElement.classList.add('bubble-pulse-vote');
          setTimeout(() => {
            adElement.classList.remove('bubble-pulse-vote');
          }, 1000);
        }
      }
    } catch (error) {
      if (error.message === 'Already voted for this ad') {
  ...
  // Update bubble size based on vote count
  const updateBubbleSizeByVotes = (element, voteCount) => {
    // Base scale from ad creation (typically between 0.95 and 1.05)
    const ad = ads.find(a => a.element === element);
    if (!ad) return;
    
    const baseScale = ad.baseScale || 1;
    
    // Calculate vote scale: 0 votes = 100%, 75 votes = 150% (2% increase per vote)
    const MAX_VOTES = 75;
    const voteScale = Math.min(1 + (voteCount / MAX_VOTES) * 0.5, 1.5);
    
    // Combine base scale with vote scale
    const newScale = baseScale * voteScale;
    
    // Apply the new scale
    element.style.transform = `scale(${newScale})`;
    
    // Store the vote scale for later use
    ad.voteScale = voteScale;
  };

  // Modify the updateBubblePosition to maintain vote scaling
  const updateBubblePosition = (ad) => {
    ...
    // Add subtle wobble effect on movement while preserving vote scaling
    if (Math.abs(ad.vx) > 0.1 || Math.abs(ad.vy) > 0.1) {
      const wobble = Math.sin(Date.now() * 0.01) * 0.03;
      const voteScale = ad.voteScale || 1;
      ad.element.style.transform = `scale(${ad.baseScale * voteScale * (1 + wobble)})`;
    }
  };
