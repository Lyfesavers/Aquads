...
// Optimize window resize handler with throttling (existing function)
function handleResize() {
  // Existing code
}

// Replace original handleResize with throttled version
const throttledHandleResize = throttle(handleResize, 100); 

// Use requestAnimationFrame for smooth animations 
function animateBubbles() {
  // This will be called by requestAnimationFrame
  // Move any animation logic here from render loop
...
function optimizedCollisionDetection(ads) {
  // Divide the screen into a grid
  const gridSize = 100; // Size of each grid cell
  const grid = {};
  
  // Place each ad in its grid cell
  ads.forEach(ad => {
    const cellX = Math.floor(ad.x / gridSize);
    const cellY = Math.floor(ad.y / gridSize);
    const cellKey = `${cellX},${cellY}`;
    
    if (!grid[cellKey]) {
      grid[cellKey] = [];
    }
    grid[cellKey].push(ad);
  });
  ...
  ads.forEach(ad1 => {
    const cellX = Math.floor(ad1.x / gridSize);
    const cellY = Math.floor(ad1.y / gridSize);
    
    // Check in current and adjacent cells
    for (let i = -1; i <= 1; i++) {
      for (let j = -1; j <= 1; j++) {
        const checkCellKey = `${cellX + i},${cellY + j}`;
        if (!grid[checkCellKey]) continue;
        
        grid[checkCellKey].forEach(ad2 => {
          // Skip if same ad or already processed this pair
          if (ad1 === ad2) return;
          const pairKey = ad1.id < ad2.id ? `${ad1.id}-${ad2.id}` : `${ad2.id}-${ad1.id}`;
          if (processed.has(pairKey)) return;
          
          // Check for collision
          handleCollision(ad1, ad2);
