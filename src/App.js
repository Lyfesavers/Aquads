...
function App() {
  ...
  // Add a useEffect that monitors points and triggers the animation at 3000
  useEffect(() => {
    // Exit early if user isn't logged in or no pointsInfo available
    if (!currentUser) return;
    
    // Check if the user has stored points information
    const checkPointsMilestone = async () => {
      try {
        ...
        // Define launch timestamp of the treasure hunt feature (set to current time for testing)
        const treasureHuntLaunchTime = new Date('2023-03-31T00:00:00Z').getTime();
        
        // Get the last points check timestamp from localStorage
        const lastPointsCheck = localStorage.getItem('lastPointsCheck');
        const lastPointsAmount = localStorage.getItem('lastPointsAmount');
        
        // Current timestamp
        const now = new Date().getTime();
        
        // If no previous check, store current values and exit
        if (!lastPointsCheck || !lastPointsAmount) {
          localStorage.setItem('lastPointsCheck', now.toString());
          localStorage.setItem('lastPointsAmount', currentPoints.toString());
          return;
        }
        
        // Parse stored values
        const lastCheck = parseInt(lastPointsCheck, 10);
        const lastAmount = parseInt(lastPointsAmount, 10);
        
        // Only process if:
        // 1. The last check was after the feature launch
        // 2. The user previously had less than 3000 points
        // 3. They now have at least 3000 points
        // 4. They haven't already celebrated this milestone
        if (
          lastCheck >= treasureHuntLaunchTime && 
          lastAmount < 3000 && 
          currentPoints >= 3000 && 
          !hasReachedMilestone
        ) {
          // Get whether this milestone was previously celebrated from localStorage
          const celebrated3kPoints = localStorage.getItem('celebrated3kPoints');
          
          // Only show animation if not previously celebrated
          if (!celebrated3kPoints) {
            setShowRewardAnimation(true);
            // Hide after 10 seconds
            setTimeout(() => setShowRewardAnimation(false), 10000);
            // Mark as celebrated so it doesn't show again
            localStorage.setItem('celebrated3kPoints', 'true');
        ...
        logger.error('Error checking points milestone:', error);
      }
    };
    
    // Run on mount and whenever currentUser changes
    checkPointsMilestone();
    
    // Also set up an interval to periodically check (every 2 minutes)
    const intervalId = setInterval(checkPointsMilestone, 120000);
    
    return () => clearInterval(intervalId);
  }, [currentUser, hasReachedMilestone]);
  ...
  // Handle reward claim when user reaches 3000 points
  const handleClaimReward = () => {
    setShowRewardAnimation(false);
    
    // Show a notification about the claimed reward
    showNotification("Congratulations! You've unlocked exclusive benefits with 3000 points!", "success");
    
    // Record the milestone achievement with timestamp
    const now = new Date().getTime();
    localStorage.setItem('celebrated3kPoints', 'true');
    localStorage.setItem('pointsMilestoneClaimedAt', now.toString());
    
    // Optionally notify the server that user has seen this milestone
    if (currentUser && currentUser.token) {
      try {
        fetch(`${API_URL}/api/points/milestone-reached`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${currentUser.token}`
          ...
          body: JSON.stringify({ 
            ...
            claimedAt: now,
            // Track that this was from the new treasure hunt feature
            source: 'treasure_hunt_launch'
          })
        });
      } catch (error) {
        logger.error('Error recording milestone achievement:', error);
      }
    }
  };
  ...
  return (
    <AuthContext.Provider value={{ currentUser, setCurrentUser }}>
      ...
      {/* 3D Reward Animation for 3000 Points Milestone */}
      {showRewardAnimation && (
        <div className="reward-animation-container" style={{
          position: 'fixed',
          top: 0,
          left: 0,
          width: '100%',
          ...
          {Array.from({ length: 150 }).map((_, index) => (
            <motion.div
              ...
              style={{
                ...
                borderRadius: '50%',
                background: `rgb(${Math.floor(Math.random() * 100 + 155)}, ${Math.floor(Math.random() * 100 + 155)}, ${Math.floor(Math.random() * 255)})`,
                boxShadow: `0 0 ${Math.random() * 10 + 5}px rgba(${Math.floor(Math.random() * 100 + 155)}, ${Math.floor(Math.random() * 100 + 155)}, ${Math.floor(Math.random() * 255)}, 0.8)`
              }}
            />
          ))}
          
          {/* Central 3D spinning reward badge */}
          <motion.div
            initial={{ 
              scale: 0,
              rotateY: 0,
            ...
            style={{
              ...
              boxShadow: '0 0 50px rgba(30, 60, 114, 0.8), 0 0 100px rgba(42, 82, 152, 0.4)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
          ...
          >
            ...
            >
              3000 POINTS ACHIEVED!
            </motion.div>
          </motion.div>
          
          {/* Congratulatory text with prize info */}
          <motion.div
          ...
          >
            ...
            <p style={{ 
              ...
              You've reached the 3000 points milestone! Keep going to unlock more rewards and benefits.
            </p>
            <motion.button
