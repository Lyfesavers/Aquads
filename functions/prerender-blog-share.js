...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const shareUrl = `https://www.aquads.xyz/share/blog/${blogId}`;
    
    return {
      statusCode: 200,
      body: getBlogHtml(blog, description, seoUrl, shareUrl),
      headers: { 'Content-Type': 'text/html' },
    };
  } catch (error) {
...
function getBlogHtml(blog, description, seoUrl, shareUrl) {
...
<head>
  ...
  <script>window.location.href='/learn?blogId=${blog._id}';</script>
</head>
<body>
  <h1>${escapedTitle}</h1>
